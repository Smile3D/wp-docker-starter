...
├── theme-template/           # шаблон темы, копируется под новым именем
│   ├── functions.php         # инфраструктура (в т.ч. фильтры ACF Local JSON)
│   └── acf-json/             # сюда ACF пишет/читает группы полей (в git)
└── wp-content/               # монтируется в контейнер (плагины, языки, темы)
```
...
Yoast SEO, Query Monitor (только dev).

- **Yoast SEO** — SEO-плагин: мета-теги, XML-карта сайта, анализ читаемости
  и ключевых слов. Мастер первичной настройки (First-time Configuration)
  отключён, чтобы не мешать при каждом `init.sh`.

**Ядро WordPress** автоматически обновляется до последней стабильной версии
при каждом запуске `init.sh` — проверка идёт после распаковки ядра и до
установки плагинов (при отсутствии сети шаг пропускается с предупреждением).

**ACF Local JSON:** в `theme-template/functions.php` подключены фильтры
`acf/settings/save_json` и `acf/settings/load_json` на папку `acf-json/`
активной темы. Группы полей, отредактированные в админке, автоматически
сохраняются туда в виде `.json` и подхватываются на новом окружении —
папку `acf-json/` коммитим в git. Конкретные группы полей в шаблоне не
лежат: `theme-template/acf-json/` пустая (только `.gitkeep`).
